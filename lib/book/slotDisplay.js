...
/** Compact chip label for mobile date strip — e.g. "Fri 26". */
export function formatSlotDateLabelCompact(iso, timeZone) {
...
/** Long heading for selected day — e.g. "Friday, June 26". */
export function formatSlotDateLabelLong(iso, timeZone) {
  return new Date(iso).toLocaleDateString("en-US", {
    weekday: "long",
    month: "long",
    day: "numeric",
...
export function formatDisplayTimeZoneLabel(timeZone) {
  if (timeZone === "America/Los_Angeles") return "Pacific Time";
  return timeZone.replace(/_/g, " ");
}

export function formatDisplayTimeZoneShortName(timeZone) {
  try {
    const parts = new Intl.DateTimeFormat("en-US", {
      timeZone,
      timeZoneName: "short",
