...
/** Custom pixel event (e.g. retarget_collateral_*). */
export function metaTrackCustom(eventName, params = {}, eventId) {
  if (!shouldEnableMetaPixelClient()) return;
  ensureMetaFbqBootstrap();
  const pathKey = typeof window !== "undefined" ? window.location.pathname : "ssr";
  const key = eventId
    ? `custom:${eventName}:${eventId}`
    : `custom:${eventName}:${pathKey}`;
  if (dedupeKeys.has(key)) return;
  dedupeKeys.add(key);
  const fbq = typeof window !== "undefined" ? window.fbq : undefined;
  if (typeof fbq !== "function") return;
  ...
  if (eventId) {
    ...
    fbq("trackCustom", eventName, cleaned);
  }
}
