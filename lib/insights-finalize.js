...
export async function forwardLeadWebhook({ webhookEnv, payload, logTag }) {
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[${logTag}] Webhook failed`, res.status, text);
      return { ok: false, data: null };
    }
    let data = null;
    if (text) {
...
/** DF Income deck signup → CRM marketing-site webhook. Returns parsed CRM JSON when available. */
export async function forwardDeckSignupWebhook(payload, logTag = "book-deck-signup") {
  const url =
    process.env.DF_INCOME_DECK_SIGNUP_WEBHOOK_URL?.trim() ||
    (process.env.CRM_API_BASE_URL
      ? `${process.env.CRM_API_BASE_URL.replace(/\/$/, "")}/api/public/marketing-site/df-income-deck-signup`
      : "");
  const secret =
    process.env.DF_INCOME_DECK_SIGNUP_WEBHOOK_SECRET?.trim() ||
    process.env.CRM_BOOK_PROXY_SECRET?.trim() ||
    "";
  return forwardLeadWebhook({
    webhookEnv: { url, secret },
    payload,
    logTag,
  });
}
...
/**
 * Multi-org WorkOS users (staff testing Google/LinkedIn on the book LP) fail
 * `authenticateWithCode` with this shape. Public callback auto-picks an org.
 */
export function organizationSelectionRequiredFromError(error) {
  const code = error?.rawData?.code;
  if (code !== "organization_selection_required") return null;
  const pendingAuthenticationToken = error?.rawData?.pending_authentication_token;
  if (typeof pendingAuthenticationToken !== "string" || pendingAuthenticationToken.length === 0) {
    return null;
  }
  const rawOrgs = error?.rawData?.organizations;
  const organizations = Array.isArray(rawOrgs)
    ? rawOrgs
        ...
        .filter((org) => org.id)
    : [];
  return { pendingAuthenticationToken, organizations };
}
